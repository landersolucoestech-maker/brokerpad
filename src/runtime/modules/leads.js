(() => {
  ...
  const initialLeads = [
    {
      id: 'LD-1001', customerId: 'CUS-1001', contactName: 'Alex Morgan', email: 'alex@example.com', phone: '(305) 555-0181',
      origin: 'Los Angeles, CA', destination: 'Miami, FL', vehicleYear: '2024', vehicleMake: 'Tesla', vehicleModel: 'Model 3',
      status: 'Quoted', quoteAmount: 1275, source: 'Google Ads', assignedTo: 'Sales Team', priority: 'Normal', notes: '',
      createdAt: '2026-08-26T11:50:00.000Z', updatedAt: '2026-08-26T11:54:00.000Z',
    },
    {
      id: 'LD-1002', customerId: '', contactName: 'David Chen', email: 'david@example.com', phone: '',
      origin: 'Newark, NJ', destination: 'Dallas, TX', vehicleYear: '2023', vehicleMake: 'BMW', vehicleModel: 'X5',
      status: 'Follow-up', quoteAmount: 1080, source: 'Referral', assignedTo: 'Sales Team', priority: 'High', notes: '',
      createdAt: '2026-08-26T11:20:00.000Z', updatedAt: '2026-08-26T11:38:00.000Z',
    },
  ];
  ...
  const statusClass = (status) => ({
    New: 'gray', Contacted: 'blue', Quoted: 'blue', 'Follow-up': 'amber', Won: 'green', Lost: 'red',
  }[status] || 'gray');
  ...
  function save(leads) {
    ...
    api.events.emit('leads:changed', { count: leads.length });
  }
  ...
  function openEditor(lead, onCommit) {
    ...
    const customerOptions = customers.map((customer) =>
      `<option value="${escapeHtml(customer.id)}" ${customer.id === current.customerId ? 'selected' : ''}>${escapeHtml(customer.name)} · ${escapeHtml(customer.id)}</option>`
    ).join('');
    ...
    layer.innerHTML = `
      <div class="bp-runtime-modal bp-runtime-modal-wide" role="dialog" aria-modal="true" aria-labelledby="bpLeadModalTitle">
        <div class="bp-runtime-modal-head">
          <div><h3 id="bpLeadModalTitle">${creating ? 'New Lead' : 'Edit Lead'}</h3><p>${creating ? 'Create a sales lead and transport request.' : escapeHtml(current.id)}</p></div>
          <button type="button" class="bp-runtime-close" data-lead-close aria-label="Close">×</button>
        </div>
        <form id="bpLeadForm" class="bp-runtime-form">
          <div class="bp-runtime-grid">
            ...
            <label><span>Quote amount</span><input name="quoteAmount" type="number" min="0" step="0.01" value="${current.quoteAmount}"></label>
          ...
          <div class="bp-runtime-modal-foot">
            ${creating ? '' : '<button type="button" class="btn danger" data-lead-delete>Delete</button>'}
            <span class="bp-runtime-spacer"></span>
            <button type="button" class="btn" data-lead-close>Cancel</button>
            <button type="submit" class="btn primary">${creating ? 'Create Lead' : 'Save Changes'}</button>
          </div>
        </form>
      </div>`;
    ...
    form.addEventListener('submit', (event) => {
      ...
      onCommit('save', normalize({ ...current, ...values, quoteAmount: Number(values.quoteAmount), updatedAt: now() }), creating);
      closeModal();
    });
  ...
  function install() {
    ...
    const render = () => {
      ...
      const rows = leads.filter((lead) => {
        ...
        return [lead.id, lead.contactName, lead.email, lead.phone, lead.origin, lead.destination, lead.vehicleYear, lead.vehicleMake, lead.vehicleModel, lead.source, lead.assignedTo]
      ...
      tbody.innerHTML = rows.length ? rows.map((lead) => `
        <tr data-lead-id="${escapeHtml(lead.id)}">
          ...
          <td>${lead.quoteAmount ? escapeHtml(money(lead.quoteAmount)) : '—'}</td>
          <td>${escapeHtml(lead.source)}</td>
          <td>${escapeHtml(relative(lead.updatedAt))}</td>
          <td><button type="button" class="btn ghost" data-lead-edit="${escapeHtml(lead.id)}">Edit</button></td>
        </tr>`).join('') : '<tr><td colspan="8" class="secondary" style="padding:18px;text-align:center">No leads match the current filters.</td></tr>';
    };
