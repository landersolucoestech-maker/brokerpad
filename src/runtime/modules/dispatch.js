(() => {
  ...
  function openManage(order, onCommit) {
    ...
    const carrierOptions = available.map((carrier) => `
      <option value="${escapeHtml(carrier.id)}" ${carrier.id === order.carrierId ? 'selected' : ''}>
        ${escapeHtml(carrier.name)} · USDOT ${escapeHtml(carrier.usdot || '—')}${isEligibleCarrier(carrier) ? '' : ' · NOT ELIGIBLE'}
      </option>`).join('');
    ...
    layer.innerHTML = `
      <div class="bp-runtime-modal" role="dialog" aria-modal="true">
        <div class="bp-runtime-modal-head">
          <div><h3>Dispatch ${escapeHtml(order.id)}</h3><p>${escapeHtml(order.origin)} → ${escapeHtml(order.destination)} · ${escapeHtml(order.vehicle)}</p></div>
        ...
        <form id="bpDispatchForm" class="bp-runtime-form">
          <div class="bp-runtime-grid">
            <label class="bp-runtime-span-2"><span>Carrier</span><select name="carrierId"><option value="">Unassigned</option>${carrierOptions}</select></label>
          ...
          <div class="bp-runtime-modal-foot">
            ${order.carrierId || order.carrierName ? '<button type="button" class="btn danger" data-dispatch-unassign>Unassign</button>' : ''}
    ...
    form.addEventListener('submit', (event) => {
      ...
      const selected = carriers.find((carrier) => carrier.id === values.carrierId);
      const error = layer.querySelector('#bpDispatchFormError');

      if (selected && !isEligibleCarrier(selected)) {
        error.textContent = 'This carrier is not dispatch eligible. Authority must be Active, insurance Verified, approval Approved and risk cannot be High.';
        error.hidden = false;
        return;
      }
  ...
  function install() {
    ...
    const render = () => {
      ...
      }).join('') : '<tr><td colspan="6" class="secondary" style="padding:18px;text-align:center">No active dispatch orders match the current search.</td></tr>';
    };
